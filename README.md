...
* Домен управление устройствами: все, что связано с управлением устройствами в доме. Включение/выключение устройств и посылка иных команд устройствам, например, установка желаемой температуры
* Домен телеметрии: все, что связано с получением данных с датчиков. Получение данных с датчиков, просмотр исторических данных датчиков
...
[Microservices ER diagram](./diagrams/Microservices_ER.puml)
...
> вместо доменов описал общую функциональность - тут стоит поправить.

Поправил. В этом же файле, раздел Домены

> Кстати, в чем идея разделения Telemetry History Service и Telemetry Service? почему нельзя получать историю из того же сервиса?

Идея была разделить ответственности: Telemetry Service просто собирает данные, а Telemetry History Service данные хранит в удобной для этого базе, например, колоночной.
Также разделяется нагрузка, запросы исторических данных не мешают работе по получению данных с счетчиков.

> диаграмма компонентов не отрендерилась, перепроверь пожалуйста

Вроде бы открывается нормально, попробовал даже на стороннем ресурсе открыть, на всякий случай прикладываю [png файл](./diagrams/Microservices_Component.png)

> просьба объединить все диаграммы в одну и добавить связей между сущностями (как минимум не хватает связей между Sensor, Device; и другими)

Поправил: [ER диаграмма](./diagrams/Microservices_ER.puml)